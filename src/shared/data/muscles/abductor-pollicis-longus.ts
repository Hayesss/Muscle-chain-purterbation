import type { MuscleEntry } from "../types";

export const abductor_pollicis_longus: MuscleEntry = {
  id: "abductor-pollicis-longus",
  name: { zh: "拇长展肌", latin: "Musculus abductor pollicis longus" },
  category: "upper-limb",
  view: ["back"],
  svgPath: {
    "back": "M175,80 Q200,75 225,80 L225,110 Q200,115 175,110 Z"
  },
  origin: "尺骨后面/骨间膜后面/桡骨后面",
  insertion: "第一掌骨底外侧/大多角骨",
  innervation: "桡神经深支 (C7-C8)",
  bloodSupply: "骨间后动脉",
  primaryActions: ["拇指外展", "腕关节外展/伸展（辅助）", "桡尺远侧关节稳定"],
  secondaryActions: ["拇伸展辅助"],
  joints: ["腕关节", "拇指腕掌关节"],
    chains: [
    {
      chainId: "nerve_synergy_brachial_plexus",
      position: "middle",
      upstream: ["abductor-digiti-minimi", "abductor-pollicis-brevis"],
      downstream: ["adductor-pollicis", "anconeus"],
    },
    {
      chainId: "func_synergy_wrist_extension",
      position: "middle",
      upstream: ["extensor-carpi-radialis-brevis", "extensor-carpi-radialis-longus"],
      downstream: ["extensor-carpi-ulnaris", "extensor-digiti-minimi"],
    },
    {
      chainId: "segment_synergy_C7_C8",
      position: "middle",
      upstream: ["anconeus"],
      downstream: ["articularis-cubiti"],
    },
  ],
  triggerPoints: [],
  interventions: {
    manualTherapy: ["触诊评估", "软组织松解"],
    dryNeedling: "临床评估后决定。",
    stretching: [{ position: "温和牵伸位", duration: "20-30秒", caution: "避免过度牵拉" }],
    strengthening: ["渐进抗阻训练"],
    prognosis: "预后良好，配合康复训练可恢复。",
    redFlags: ["进行性加重", "伴随神经症状", "不明原因体重下降"],
  },
  relatedDiagnoses: [],
  relatedTechniques: [],
  relatedSymptomRegions: [],
};
